...
/**
 * Home object for domain model class Shoes.
 * @see cn.Shoes
 * @author Hibernate Tools
 */
public class ShoesHome {

	private static final Log log = LogFactory.getLog(ShoesHome.class);
	...
	public void persist(Shoes transientInstance) {
		log.debug("persisting Shoes instance");
		try {
	...
	public Shoes merge(Shoes detachedInstance) {
		...
		try {
			Shoes result = (Shoes) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Shoes findById(java.lang.String id) {
		...
		try {
			Shoes instance = (Shoes) sessionFactory.getCurrentSession().get("cn.Shoes", id);
			if (instance == null) {
	...
	public List findByExample(Shoes instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("cn.Shoes").add(Example.create(instance))
					.list();
